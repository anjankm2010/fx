...
input int Take_Profit = 51; // Take Profit (pips)
...
input int Ind1Param1 = -76; // Level
static input string Ind2 = "------------";// ----- Momentum -----
input int Ind2Param0 = 42; // Period
input double Ind2Param1 = 59.0161; // Level

static input string ExpertSettings__ = "------------"; // ------ Expert Settings ------
static input int Magic_Number = 40469993; // Magic Number
...
void ManageClose()
  {
   double ind1val1 = iWPR(NULL,0,Ind1Param0,1);
   double ind1val2 = iWPR(NULL,0,Ind1Param0,2);
   ...
   double ind2val1 = iMomentum(NULL,0,Ind2Param0,PRICE_CLOSE,1);
   double ind2val2 = iMomentum(NULL,0,Ind2Param0,PRICE_CLOSE,2);
   bool ind2long  = ind2val1 < Ind2Param1 - sigma && ind2val2 > Ind2Param1 + sigma;
   bool ind2short = ind2val1 > 200 - Ind2Param1 + sigma && ind2val2 < 200 - Ind2Param1 - sigma;

   if(posType==OP_BUY && (ind1long || ind2long))
      ClosePosition();
   else if(posType==OP_SELL && (ind1short || ind2short))
      ClosePosition();
  }
//+------------------------------------------------------------------+
//|                                                                  |
...
/*STRATEGY CODE {"properties":{"entryLots":0.1,"stopLoss":94,"takeProfit":51,"useStopLoss":true,"useTakeProfit":true,"isTrailingStop":false},"openFilters":[{"name":"Accelerator Oscillator","listIndexes":[5,0,0,0,0],"numValues":[0,0,0,0,0,0]}],"closeFilters":[{"name":"Williams' Percent Range","listIndexes":[4,0,0,0,0],"numValues":[24,-76,0,0,0,0]},{"name":"Momentum","listIndexes":[5,3,0,0,0],"numValues":[42,59.0161,0,0,0,0]}]} */
